...
<!-- LightGallery CSS -->
<link type="text/css" rel="stylesheet" href="https://cdn.jsdelivr.net/npm/lightgallery@2.7.2/css/lightgallery-bundle.min.css" />
                                ...
                                <div class="lightgallery-container" id="lightgallery-<?php echo $project['id']; ?>" style="display: block;">
                                    <?php foreach ($images as $imgIndex => $imagePath): ?>
                                        <a href="<?php echo assets(htmlspecialchars(trim($imagePath))); ?>"
                                            class="gallery-item" data-sub-html="<?php echo htmlspecialchars($project['title']); ?> - Image <?php echo $imgIndex + 1; ?>">
                                ...
                                <div class="project-links">
                                    <?php if (!empty($project['host_url'])): ?>
                                        <a href="<?php echo htmlspecialchars($project['host_url']); ?>" target="_blank" class="project-link">
                                            <i class="fas fa-external-link-alt"></i> <?=__('live_demo')?>
                                        </a>
                                    <?php endif; ?>
                                    <?php if (!empty($project['github_url'])): ?>
...
<script>
    ...
    function updateMainImage(projectId) {
        ...
        if (mainImage) {
            mainImage.src = assets(gallery.images[gallery.currentIndex]);
